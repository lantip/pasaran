...
JAWA_EPOCH = 1745719.5
JAWA_WEEKDAYS = (
    "Minggu", "Senen", "Seloso", "Rebo",
    "Kemis", "Jemuwah", "Sebtu")
...
class Calverter:
    ...
    def leap_jawa(self, year):
        "Is a given year a leap year in the Javanese calendar ?"

        return (((year * 11) + 14) % 30) < 11

    def jd_to_jawa(self, jd):
        "Calculate Javanese date from Julian day"
        jd = math.floor(jd) + 0.5
        year = int(math.floor(((30 * (jd - ISLAMIC_EPOCH)) + 10646) / 10631)) + 512
        month = int(min(12, math.ceil(
            (jd - (29 + self.islamic_to_jd(year - 512, 1, 1))) / 29.5) + 1))
        day = int(jd - self.islamic_to_jd(year - 512, month, 1)) + 1
        return year, month, day
